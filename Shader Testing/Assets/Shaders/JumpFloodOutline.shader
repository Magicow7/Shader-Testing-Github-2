...
{
    //NOTES ON THIS SHADER
    // a texel is the fundemental unity of a texture, like a pixel for a texture
    //jump flood is insanely fast, Log(n)
    //jump flood has a list of seeds, and a step length
    //for each seed, we we populate the point (step length) away on all cardinal directions, including minor ones, with the same data,
    //and add those points to the seeds, then half the step length. and repeate with all seeds.

    //Graphics.blit is incridible
    Properties
    {
    ...
    SubShader
    {
        ...
        //this pass is just setup I think
        Pass // 0
        {
            Name "INNERSTENCIL"
        ...
        //this pass is to fill the buffer with the objects to be outlined
        Pass // 1
        {
            Name "BUFFERFILL"
        ...
        //sets up outline, uses sobel filter
        Pass // 2
        {
            Name "JUMPFLOODINIT"
            ...
            float2 frag (v2f i) : SV_Target {
                ...
                half3x3 values;
                //this unrolls loops in compilation, probably works as long as we are using constants or static vars.
                UNITY_UNROLL
                for(int u=0; u<3; u++)
                {
                    UNITY_UNROLL
        ...
        //jump flood on one axis, will be called twice, technically faster
        Pass // 4
        {
        ...
        //actually draws the outline, based on the data
        Pass // 5
        {
